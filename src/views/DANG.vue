<template>
      ...
      <div v-if="activeTool" class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <div class="flex gap-2">
            <button
              :class="['input text-sm text-center w-full', unit === 'imperial' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 border-gray-300']"
              @click="unit = 'imperial'"
            >Imperial (in)</button>
            <button
              :class="['input text-sm text-center w-full', unit === 'metric' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 border-gray-300']"
              @click="unit = 'metric'"
            >Metric (mm)</button>
          </div>
        </div>
  ...
  // Auto-update unit when switching tools
  ...
  const saveToLibrary = async () => {
    ...
    try {
      ...
      if (!tool_id || !generatedDescription.value || existing.size > 0) {
        saveStatus.value = existing.size > 0
          ? '❌ Tool ID already exists.'
          : '❌ Incomplete tool info.'
